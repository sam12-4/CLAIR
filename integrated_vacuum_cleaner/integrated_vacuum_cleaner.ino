...
void turnLeft() {
  ...
  // Right motor forward, left motor stopped or slower
  digitalWrite(IN1, HIGH);
  digitalWrite(IN2, LOW);
  digitalWrite(IN3, LOW);
  digitalWrite(IN4, LOW);
  
  // Apply speed to right motor, zero or reduced speed to left
  ledcWrite(PWM_CHANNEL_A, motorSpeed);
  ledcWrite(PWM_CHANNEL_B, 0);
...
void turnRight() {
  ...
  // Left motor forward, right motor stopped or slower
  digitalWrite(IN1, LOW);
  digitalWrite(IN2, LOW);
  digitalWrite(IN3, HIGH);
  digitalWrite(IN4, LOW);
  
  // Apply speed to left motor, zero or reduced speed to right
  ledcWrite(PWM_CHANNEL_A, 0);
  ledcWrite(PWM_CHANNEL_B, motorSpeed);
...
void turnFanOn() {
  ...
  // First apply speed to the fan
  ledcWrite(FAN_PWM_CHANNEL, fanSpeed);
  Serial.println("Fan PWM set to: " + String(fanSpeed) + " on channel: " + String(FAN_PWM_CHANNEL) + " pin: " + String(FAN_EN));
  
  // Then set fan direction (forward)
  digitalWrite(FAN_IN1, HIGH);
  digitalWrite(FAN_IN2, LOW);
  Serial.println("Fan control pins - IN1:" + String(digitalRead(FAN_IN1)) + " IN2:" + String(digitalRead(FAN_IN2)));
